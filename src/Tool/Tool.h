...
namespace ToolFramework{
  ...
  public:
    ...
    std::string GetName() {return m_unique_name;};
    void SetName(std::string name) {m_unique_name=name;};
  ...
  protected:
    ...
    template <typename T>  void Log(T message, int messagelevel, int verbosity){m_log->Log("-"+m_unique_name+"-: "+message,messagelevel,verbosity);}
    template <typename T>  void Log(T message, int messagelevel=0){m_log->Log("-"+m_unique_name+"-: "+message,messagelevel,m_verbose);}  ///< Logging fuction for printouts. @param message Templated message string. @param messagelevel The verbosity level at which to show the message. Checked against internal verbosity level
    ...
    bool InitialiseConfiguration(std::string configfile=""){ ///< Inisialisation of m_varaiables first from global vars and then local config file to overlaod. @param configfile configfile path.
      ...
      if(reinterpret_cast<DataModelBase*>(m_data)->vars.Get(m_unique_name, vars_json)) m_variables.JsonParser(vars_json);
    ...
    void ExportConfiguration(){  ///< Export of local m_variables Store to matamodel vars for central exporting of configuration to data file.
      ...
      reinterpret_cast<DataModelBase*>(m_data)->vars.Set(m_unique_name,vars_json);
    }
